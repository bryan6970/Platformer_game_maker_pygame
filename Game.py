    ...
    def displayText(win, text: str,color=(255,255,255),position='center', display_resolution = None, font="Impact", size = 30, time_on_screen= None) -> None:
        ...
        # Render the text onto a new Surface object
    ...
    class Game:
        ...
        def run_game(self, movement_input_list, gun_input_list, running_fps: float):
            ...
            # Check if a bullet hit anyone
            PLAYER1_HIT_ENEMY, self.Player1_GUN.LEFT_BULLET_RECTS, self.Player1_GUN.RIGHT_BULLET_RECTS = \
                self._check_collide_player(1, self.Player1_GUN.LEFT_BULLET_RECTS, self.Player1_GUN.RIGHT_BULLET_RECTS)
    
            PLAYER2_HIT_ENEMY, self.Player2_GUN.LEFT_BULLET_RECTS, self.Player2_GUN.RIGHT_BULLET_RECTS = \
                self._check_collide_player(0, self.Player2_GUN.LEFT_BULLET_RECTS, self.Player2_GUN.RIGHT_BULLET_RECTS)
    ...
    class Gun:
        ...
        def _move_bullets(self):
            for i, bullet in enumerate(self.LEFT_BULLET_RECTS):
                self.LEFT_BULLET_RECTS[i][0] -= self.init_obj.BULLET_SPEED
    
            for i, bullet in enumerate(self.RIGHT_BULLET_RECTS):
                self.RIGHT_BULLET_RECTS[i][0] += self.init_obj.BULLET_SPEED
    
            self.bullets.extend(self.LEFT_BULLET_RECTS)
            self.bullets.extend(self.RIGHT_BULLET_RECTS)
